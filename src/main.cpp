...
{
    std::cout << "Finding max move: a=" << alpha << " b=" << beta << std::endl;
    ...
    // Generate all possible moves.  TODO: Change the game_state pointers to a smart pointer
    std::shared_ptr<std::vector<game_state *>> next_possible_states = game_state_instance.generate_possible_next_states();
...
int main()
{
    ...
    action next_move = find_next_move(*game_state_instance);
}